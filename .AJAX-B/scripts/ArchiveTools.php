...
class archive
{
	...
	{
		$this->options = array (
			'basedir' => ".",   // dossier racine des adresses
			'name' => $name,    // nom de l'archive
			'prepend' => "",    // dossier racine dans l'archive
			'inmemory' => 0,    // Uniquement en RAM ou vraiment sur le HDD
			'overwrite' => 0,   // ecrase ou pas les fichier deja existant
			'recurse' => 1,     // Compression des dossiers seulement sur un niveau ou recursif
			'storepaths' => 1,  // Conserve l'arborescence ou pas
			'followlinks' => 0, // 1 compresse la cible des liens, 0 compresse les liens symboliques
			'level' => 6,       // niveau de compression (9 = hard)
			'method' => 1,      // ZIP : 0 ne compresse rien
			'sfx' => "",        // ?
			'type' => "",       // ?
			'comment' => ""     // commentaire (unniquement pour les *.ZIP)
		);
		$this->files = array ();        // Liste des source a compresser
		$this->exclude = array ();      // fichier et dossier exclu de l'archive
		$this->storeonly = array ();    // fichier et dossier ajouté sans compression
		$this->error = array ();        // Liste des erreur rencontrées
	}
